(ns langchain.main
  "Kotodama WASM entrypoint for the langchain clean-room actor (L4) — Clojure port.
  ...
  facts (`langchain.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "langchain")
(def tier "L4")
...
(def entity-specs
  [{:entity "Agent"   :plural "agents"   :id-prefix "langchai_age"
    :fields [:name :role :modelRef :status]              :required [:name :role]
    :coerce {}                       :refs {}}
   {:entity "Task"      :plural "tasks"      :id-prefix "langchai_tas"
    :fields [:agentId :goal :status :priority] :required [:goal :status]
    :coerce {:priority :int}          :refs {:agentId "Agent"}}
   {:entity "Message"     :plural "messages"     :id-prefix "langchai_mes"
    :fields [:fromAgentId :toAgentId :performative :content]        :required [:performative :content]
    :coerce {} :refs {}}
   {:entity "Tool" :plural "tools" :id-prefix "langchai_too"
    :fields [:name :description :schemaRef]          :required [:name :description]
    :coerce {}                       :refs {}}
   {:entity "Memory"   :plural "memories"   :id-prefix "langchai_mem"
    :fields [:agentId :kind :contentRef]   :required [:kind]
    :coerce {}        :refs {:agentId "Agent"}}
   {:entity "Plan"    :plural "plans"    :id-prefix "langchai_pla"
    :fields [:taskId :steps :status]  :required [:steps :status]
    :coerce {}          :refs {:taskId "Task"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"langchain.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "langchain-compat" :tier tier :entities entities} 200])
...
(defn start! [] :langchain-compat/ready)
